...
fn agent_api_value_failures_return_err_pairs() {
    ...
    let src = format!(
        r#"maki.api.register_tool({{
            ...
            handler = function(input, ctx)
                local parts = {{}}
                local p, perr = maki.agent.system_prompt(ctx, {{ prompt_id = "nope" }})
                parts[1] = (p == nil and perr == "unknown prompt_id: nope") and "prompt_err" or "prompt_ok"
                local t, terr = maki.agent.tools(ctx, {{ audience = "nope" }})
                parts[2] = (t == nil and terr == "unknown audience: nope") and "tools_err" or "tools_ok"
                local m, merr = maki.agent.resolve_model(ctx, {{ spec = "not-a-spec" }})
                parts[3] = (m == nil and merr ~= nil) and "model_err" or "model_ok"
                return table.concat(parts, " ")
            end
        }})"#
    );
...
/// `start` runs before permission checks: its ctx reads config/workflow/
/// audience and publishes previews, but dispatch/finish/deadline return
/// `(nil, err)`.
#[test]
